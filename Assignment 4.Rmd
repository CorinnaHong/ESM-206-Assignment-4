...
traps_line <- ggplot(traps_counts, aes(x = YEAR, y = TRAPS, group = SITE)) + 
  geom_line(aes(linetype = SITE, color = SITE))+
  theme_classic()+
  geom_point(aes(color = SITE))+
...
Merging and creating a graph with both sets of data on it
```{r}

#Merge data sets
abundance_traps <- full_join(lobster_counts, traps_counts) 

# Graph first set of data (abundance)
abundance_traps_line <- ggplot(abundance_traps, aes(x = YEAR, y = COUNT)) + 
  ...
  facet_wrap(~SITE) +
  theme_classic() +
  labs(x = "Year", y = "Annual Lobster Adundance")
...
# Need to change colors to distinguish between which line is abundance (lower one) and which are traps (higher one)
